...
var MatlabEditor =
  module.exports = {
      ...
      MatlabEditor.subscriptions.add(
        ...
        new Disposable(watchPath(MatlabEditor.getMatlabPrefDir(), {}, events => {
          for (const event of events) {
            if (event.action === 'created' && path.basename(event.path) === 'matlab.settings') {
    ...
    openInMatlab () {
      if (MatlabEditor.checkSessionStatus()) {
        ...
        if (fileExt === '.mat') {
          ...
          MatlabEditor.runCommand('edit(\'' + filePath + '\')', null)
        } else {
    ...
    updateCurrentFolderStatusbar () {
      var pathToSettings = path.join(MatlabEditor.getMatlabPrefDir(), 'matlab.settings')
      ...
      fs.readFile(pathToSettings, (err, text) => {
        if (err) {
          ...
          var cf = xmlDoc.querySelectorAll('key[name=History]')[0].getElementsByTagName('value')[0].textContent
