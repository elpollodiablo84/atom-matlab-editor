...
module.exports =
  class MatlabHelpView {
    ...
    loadView (keyword) {
      if (keyword) {
        fs.readFile(this.getHelpPath() + keyword + '.html', (err, html) => {
          if (!err) {
            var docFile = document.createElement('html')
            docFile.innerHTML = html.toString()
            ...
            for (let i = 0; i < imgList.length; i++) {
              fileName = imgList[i].src.split('\\').pop().split('/').pop()
              imgList[i].src = this.getHelpPath() + fileName
            }
    ...
    getHelpPath () {
      return atom.config.get('atom-matlab-editor.matlabRootPath') + '\\help\\matlab\\ref\\'
    }
