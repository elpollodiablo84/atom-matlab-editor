...
module.exports =
  class MatlabHelpView {
        ...
        fs.readFile(htmlPath, (err, html) => {
          if (!err) {
            ...
            for (let i = 0; i < seeAlsoLinks.length; i++) {
              seeAlsoLinks[i].firstChild.addEventListener('click', event => {
                ...
                if (link.tagName === 'CODE') {
                  this.loadView(path.join(this.getCurrentPath(), event.target.parentNode.parentNode.getAttribute('href')))
                } else if (link.tagName === 'SPAN') {
                  this.loadView(path.join(this.getCurrentPath(), event.target.parentNode.getAttribute('href')))
                }
              })
            }
              ...
              for (let j = 0; j < liArray.length; j++) {
                if (!liArray[j].firstChild.getAttribute('target')) {
                  liArray[j].firstChild.addEventListener('click', event => {
                    this.loadView(path.join(this.getCurrentPath(), event.target.getAttribute('href')))
                  })
                }
              }
            }
            ...
            for (let i = 0; i < relatedLinks.length; i++) {
              ...
              for (let j = 0; j < liArray.length; j++) {
                liArray[j].firstChild.addEventListener('click', event => {
                  console.log(path.join(this.getCurrentPath(), event.target.getAttribute('href')))
                  this.loadView(path.join(this.getCurrentPath(), event.target.getAttribute('href')))
                })
              }
            }
            ...
            for (let i = 0; i < imgList.length; i++) {
              ...
              imgList[i].src = path.join(this.getCurrentPath(), filename)
            }
    ...
    getCurrentPath () {
      if (this.htmlPath) {
        return path.dirname(this.htmlPath)
      } else {
        return this.getHelpPath()
      }
    }
